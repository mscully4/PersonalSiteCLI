...
class GooglePhotosClient(object):
    ...
    def _create_service(self, config, scopes):
        ...
        service = build(
            API_SERVICE_NAME, API_VERSION, credentials=cred, static_discovery=False
        )
        return service
    ...
    async def get_albums(self):
        ...
        while loop:
            albums = (
                self.service.albums().list(pageSize=50, pageToken=pageToken).execute()
            )
    ...
    def get_album_photos(self, album_id):
        ...
        while loop:
            resp = (
                ...
                .search(
                    body={"albumId": album_id, "pageToken": page_token, "pageSize": 50}
                )
                .execute()
            )
